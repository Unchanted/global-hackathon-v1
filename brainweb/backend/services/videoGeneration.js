...
class VideoGenerationService {
  ...
  async generateMemoryVideo(grandparentId, memoryIds, options = {}) {
    try {
      ...
      // 3. Generate voiceover using Google Cloud TTS
      const audioFile = await this.generateVoiceover(script.fullScript, options);
      console.log('🎤 Generated voiceover:', audioFile);

      // 4. Create video slides with images and text
      ...
      const videoFile = await this.renderVideoWithRemotion(slides, audioFile, memories, script, options);
...
Respond in JSON format:
{
  ...
  async generateVoiceover(script, options) {
    ...
    const audioFile = path.join(this.tempDir, 'narration.mp3');
